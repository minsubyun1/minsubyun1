![header](https://capsule-render.vercel.app/api?type=waving&color=gradient&height=300&section=header&text=MINSUB%20YUN's%20Github)

## PROJECT 🌊

|   프로젝트명    |                                   설명                                    |        기간         |      역할       |                                 링크                                  |
|:----------:|:-----------------------------------------------------------------------:|:-----------------:|:-------------:|:-------------------------------------------------------------------:|
|    펫어웰     | [2025 kakao X groom 시즌톤] 🐾 반려동물을 잃은 뒤 보호자가 겪는 상실 극복을 돕는 기록·위로 커뮤니티 서비스 | 2025.08 ~ 2025.09 |      BE       | [펫어웰](https://github.com/9oormthon-univ/2025_SEASONTHON_TEAM_23_BE) |
| FinanceDoc |                       청년을 위한 지출 관리, 금융 상품 추천 서비스                        | 2025.08 ~ 2025.11 |      BE       |               [파이낸스닥](https://github.com/Finance-doc)               |
|   NexERP   |              [TAVE 16기 후반기 프로젝트] 기업의 모든 프로세스를 하나의 흐름으로 만드는 클라우드 솔루션 (최우수상 🏆)               | 2025.10 ~ 2026.01 | PM, INFRA, BE |                             [NexERP](https://github.com/TAVE-9RP)                              |


## ⚙ Skill (스킬)

[![My Skills](https://skillicons.dev/icons?i=java,spring,aws,docker,githubactions,mysql&perline=3)](https://skillicons.dev)


## 📄 Certificate
<li>AWS Certified Solutions Architect – Associate 📆 2025.10.31 </li>
<li>정보처리기사 📆 2024.06.18 </li>
<li>컴퓨터활용능력 1급 📆 2024.04.26 </li>
<li>ADSP 📆 2024.03.22 </li>
<li>SQLD 📆 2021.12.17 </li>


<!--
**minsubyun1/minsubyun1** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
